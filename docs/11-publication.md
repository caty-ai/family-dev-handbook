...
**リポジトリを公開するレーンは、[PB-2](#pb-2) の正本チェックリストでゲートする。完了記録には、チェックリストのヘッダーを様式の正本とする項目ごとの PASS / FAIL / N/A 表と、各項目が指定する証拠 artifact を置く。検証不能な項目は未通過とする。FAIL または未通過の項目が残る限り、公開しない。**
