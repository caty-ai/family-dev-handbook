...
4. `# CUSTOMIZE` を埋める:
   ...
   - `review-labels.yml` — **高リスク領域のリポ固有パス。課金・支出 / 対外公開・対人送信は「該当なし」でも `none` の明示宣言まで必須**（`__DECLARE_ME__` のままなら赤）
...
| `review-labels.yml` | `RISK_PATHS_REPO`（課金・対外送信のリポ固有パス・宣言必須） | `__DECLARE_ME__`（= 赤） |
