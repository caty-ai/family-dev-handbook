...
| ファイル | `with:` 入力 | 既定値 |
|---|---|---|
| `test-lint.yml` | `run_macos` / `macos_skip_reason` / `require_suite_reconciliation` / `max_skip_percent` / `node_version` / `pre_test_setup_ubuntu` / `pre_test_setup_macos` | `run_macos: true` / `require_suite_reconciliation: false`（suite 行不在は `::notice::` を出して通す。コマンド自体は reusable 側で Makefile 不在 = 赤）/ `max_skip_percent: '20'` / `node_version: ''`（runner 既定 Node）/ `pre_test_setup_*: ''`（ステップなし） |
