...
1. เปิด “บล็อกสรุปสำหรับแจกจ่าย” ใน [docs/04](docs/04-adoption.md) (เป็นข้อความประมาณ 45 บรรทัด)
...
ที่ต้องแปะมีเท่านี้ ข้างในมีเพียง rule ID แปดตระกูล (`L2-1`–`L2-6` / `L1-1`–`L1-11` / `L0-1`–`L0-9` / `FP-1`–`FP-9` / `E-1`–`E-10` / `B-1`–`B-5` / `LC-1`–`LC-5` / `R-1`–`R-6`) กับท่าทีบรรทัดเดียวของแต่ละข้อ ตัวบทของกฎไม่ได้อยู่ในนั้น **ฉบับหลักของตัวบทคือรีโปนี้ และเมื่อบทสรุปขัดกับฉบับหลัก ให้ยึดฉบับหลักเป็นหลัก** จะปรับให้เข้มขึ้นตามรีโปของคุณเองได้อย่างอิสระ แต่การผ่อนให้หลวมลงนั้นห้าม

ถ้าอยากเลิกใช้ แค่ลบราว 45 บรรทัดที่แปะไว้ ทุกอย่างก็กลับไปเหมือนเดิม ไฟล์อื่นไม่ถูกแตะเลย
...
กฎแบ่งเป็นแปดตระกูล และทุกข้อมี ID ที่ไม่เปลี่ยนแปลงกำกับไว้ ทั้งบทสรุป บทสนทนา และ Issue ต่างอ้างถึงกันด้วย ID เหล่านี้
...
เวอร์ชันปัจจุบันคือ **v0.9.2** (2026-08-11) — เป็นการปรับปรุงให้แข็งแกร่งขึ้นของชุดเทมเพลตด่านตรวจของเครื่อง ([templates/ci/](templates/ci/README.md)) โดยอุดข้อบกพร่องแบบ "เขียวแบบเงียบ ๆ" 3 จุดที่สะท้อนกลับมาจากการใช้งานจริงของการติดตั้งครั้งแรก (3 รีโป) ไว้ที่ระดับเทมเพลต ได้แก่ ① เพิ่มหมวดหมู่ประจำสำหรับขอบเขตการยืนยันตัวตน/สิทธิ์ `RISK_PATHS_AUTH` ใหม่ (เดิมเน็ตค่าเริ่มต้น `**/auth/**` ตั้งสมมติฐานว่า "โค้ดยืนยันตัวตนอยู่ในไดเรกทอรี auth/" ทำให้รีโปที่วางโครงสร้างแบบแบนมีพื้นผิวการยืนยันตัวตนทั้งหมดหลุดออกนอกเน็ต — เปลี่ยนเป็นต้องประกาศและ fail-closed) ② ทำให้การประกาศ `none` เข้มงวดเรื่องตัวพิมพ์เล็ก-ใหญ่ และตรวจสอบความสมบูรณ์ของบรรทัดประกาศ (หลายรูปแบบคั่นด้วยช่องว่าง บรรทัดคอมเมนต์ เศษเพลสโฮลเดอร์ที่หลงเหลือ ทั้งหมดขึ้นแดง) พร้อมคำเตือนรูปแบบที่ตายแล้ว ③ เพิ่มข้อควรระวังของการตรวจสอบการติดตั้งลงใน README ของ templates/ci (fixture ที่ยืนยันแล้วว่าตรวจจับได้ ปิดช่องโหว่ "เป้าหมาย 0 รายการแล้วเขียว" และใช้ร่วมกับการสแกน shebang) การปรับปรุงนี้ผ่านการรีวิวหลายที่นั่ง 4 รอบ (ทั้ง 3 ที่นั่ง GO) และซิงก์ไปยังรีโปที่ติดตั้งไปแล้ว
