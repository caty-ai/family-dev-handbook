...
ที่ต้องแปะมีเท่านี้ ข้างในมีเพียง rule ID เก้าตระกูล (`L2-1`–`L2-6` / `L1-1`–`L1-11` / `L0-1`–`L0-9` / `FP-1`–`FP-9` / `E-1`–`E-10` / `B-1`–`B-5` / `LC-1`–`LC-5` / `R-1`–`R-6` / `T-1`–`T-4`) กับท่าทีบรรทัดเดียวของแต่ละข้อ ตัวบทของกฎไม่ได้อยู่ในนั้น **ฉบับหลักของตัวบทคือรีโปนี้ และเมื่อบทสรุปขัดกับฉบับหลัก ให้ยึดฉบับหลักเป็นหลัก** จะปรับให้เข้มขึ้นตามรีโปของคุณเองได้อย่างอิสระ แต่การผ่อนให้หลวมลงนั้นห้าม
...
กฎแบ่งเป็นเก้าตระกูล และทุกข้อมี ID ที่ไม่เปลี่ยนแปลงกำกับไว้ ทั้งบทสรุป บทสนทนา และ Issue ต่างอ้างถึงกันด้วย ID เหล่านี้
...
| **T** เกณฑ์เทสต์ & CI | จะสะสมหลักฐานความถูกต้องอย่างไร | `T-1`–`T-4` | [docs/10](docs/10-test-ci-baseline.md) |
...
| [docs/10-test-ci-baseline.md](docs/10-test-ci-baseline.md) | เกณฑ์เทสต์ & CI ชั้น T — การจัดเตรียมเบื้องต้น・ค่าเริ่มต้นของ regression test・การเชื่อมกับบรีฟการมอบหมาย・fail-closed merge ภาคผนวกมีตารางแนะนำตัวรันเนอร์แบบไม่บังคับ (`T-1`–`T-4`) |
...
เวอร์ชันปัจจุบันคือ **v0.10.0** (2026-08-14) ได้เพิ่มชั้นเกณฑ์เทสต์ & CI (`T-1`–`T-4`・[docs/10](docs/10-test-ci-baseline.md)) เข้ามา — เป็นชั้นที่บัญญัติขั้นต่ำสุดให้แต่ละรีโปพิสูจน์ความถูกต้องของตัวเองต่อเนื่องด้วยเทสต์และ CI รีโปใหม่ที่มีโค้ดต้องจัดเตรียมเทสต์รันเนอร์ + CI ตั้งแต่ตอนสร้าง (แม้ยังไม่มีเทสต์สักตัวก็ต้องตั้งโครงไว้ก่อน・รีโปเดิมให้จัดเตรียมในเลนถัดไปที่แตะโค้ด) การแก้บั๊กขนาด M / L / H มีเทสต์สร้างซ้ำ (fix ก่อนขึ้นแดง・fix แล้วเขียว) เป็นค่าเริ่มต้นที่ต้องแนบมาด้วย (เหตุผลที่แนบไม่ได้เป็นรายการปิดพร้อมช่องอำนาจตัดสินของ owner) การตรวจสอบการพัฒนาของบรีฟที่มีการแก้โค้ดต้องมีหัวข้อมาตรฐาน "เทสต์ที่เพิ่ม/แก้ไขพร้อมผลการรัน" และห้าม merge ขณะ CI ยังแดง (ข้อยกเว้นมีเฉพาะสีแดงที่รู้อยู่แล้วและไม่เกี่ยวข้อง ซึ่งต้องผ่านสี่เงื่อนไขคือ ทำซ้ำได้บน base・ระบุ identity ของสีแดง・อ้างอิง Issue ที่มีทริกเกอร์การหมดอายุตาม LC-1・และได้รับอนุมัติจาก owner ที่ตรวจสอบตัวตนได้จริง) จุดเริ่มต้นคือ [caty-ai/x-collector#9](https://github.com/caty-ai/x-collector/issues/9) — รีวิวจากภายนอกชี้ว่า "ไม่มีเทสต์เลย" คือจุดอ่อนด้านความน่าเชื่อถือที่ใหญ่ที่สุด และเมื่อนำชุดเทสต์ + CI แบบ fail-closed มาใช้ ข้อบ่งชี้นั้นก็หายไปจริงตามที่วัดผลได้
