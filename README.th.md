...
ที่ต้องแปะมีเท่านี้ ข้างในมีเพียง rule ID หกตระกูล (`L2-1`–`L2-6` / `L1-1`–`L1-11` / `L0-1`–`L0-9` / `FP-1`–`FP-9` / `E-1`–`E-10` / `B-1`–`B-5`) กับท่าทีบรรทัดเดียวของแต่ละข้อ ตัวบทของกฎไม่ได้อยู่ในนั้น **ฉบับหลักของตัวบทคือรีโปนี้ และเมื่อบทสรุปขัดกับฉบับหลัก ให้ยึดฉบับหลักเป็นหลัก** จะปรับให้เข้มขึ้นตามรีโปของคุณเองได้อย่างอิสระ แต่การผ่อนให้หลวมลงนั้นห้าม
...
กฎแบ่งเป็นหกตระกูล และทุกข้อมี ID ที่ไม่เปลี่ยนแปลงกำกับไว้ ทั้งบทสรุป บทสนทนา และ Issue ต่างอ้างถึงกันด้วย ID เหล่านี้
...
เวอร์ชันปัจจุบันคือ **v0.5.0** (2026-08-06) โดยเพิ่มชั้นบรีฟการมอบหมาย (`B-1`–`B-5`, [docs/07](docs/07-delegation-brief.md)) เข้ามา — พรอมต์ที่ส่งงานให้ซับเอเจนต์กลายเป็นสัญญาสามชั้น ได้แก่ สเปกการพัฒนา การตรวจสอบตนเอง และเกณฑ์ของผู้รีวิว (แบบฟอร์มอยู่ที่ [templates/brief-template.md](templates/brief-template.md))
