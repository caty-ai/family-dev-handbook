...
1. เปิด “บล็อกสรุปสำหรับแจกจ่าย” ใน [docs/04](docs/04-adoption.md) (เป็นข้อความประมาณ 40 บรรทัด)
...
ที่ต้องแปะมีเท่านี้ ข้างในมีเพียง rule ID เจ็ดตระกูล (`L2-1`–`L2-6` / `L1-1`–`L1-11` / `L0-1`–`L0-9` / `FP-1`–`FP-9` / `E-1`–`E-10` / `B-1`–`B-5` / `LC-1`–`LC-5`) กับท่าทีบรรทัดเดียวของแต่ละข้อ ตัวบทของกฎไม่ได้อยู่ในนั้น **ฉบับหลักของตัวบทคือรีโปนี้ และเมื่อบทสรุปขัดกับฉบับหลัก ให้ยึดฉบับหลักเป็นหลัก** จะปรับให้เข้มขึ้นตามรีโปของคุณเองได้อย่างอิสระ แต่การผ่อนให้หลวมลงนั้นห้าม

ถ้าอยากเลิกใช้ แค่ลบ 40 บรรทัดที่แปะไว้ ทุกอย่างก็กลับไปเหมือนเดิม ไฟล์อื่นไม่ถูกแตะเลย
...
กฎแบ่งเป็นเจ็ดตระกูล และทุกข้อมี ID ที่ไม่เปลี่ยนแปลงกำกับไว้ ทั้งบทสรุป บทสนทนา และ Issue ต่างอ้างถึงกันด้วย ID เหล่านี้
...
เวอร์ชันปัจจุบันคือ **v0.7.1** (2026-08-07) โดยทำให้เกณฑ์จำแนกขนาดงาน (S / M / L / H / Epic, L2-1 ใน [docs/01](docs/01-milestone-loop.md)) กลายเป็นตัวบทสัญญา — ตัดสินด้วยตารางนิยามขนาดบวกสามแกน (ขอบเขตผลกระทบ ความย้อนกลับไม่ได้ ระยะเวลา) และเมื่อไม่แน่ใจให้ปัดไปทางหนักกว่า อินพุตของจำนวนที่นั่งรีวิว (L1-11) และประตูเข้า Epic (E-1) จะไม่ต้องพึ่งความรู้โดยนัยของผู้นำไปใช้อีกต่อไป ส่วน v0.7.1 เป็นการตามปรับถ้อยคำ: จัดรายการขอบเขตของการรีวิวต้นน้ำ (L1-9 ใน [docs/02](docs/02-issue-loop.md)) ให้ตรงกับระบบขนาดเดียวกัน (L / H / Epic = ฝั่งหนัก)
