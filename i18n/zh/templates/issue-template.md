...
各契约（rule ID）字段模式的正本。正文侧（docs/01〜03・05〜07）只保留最简 MUST 语句，详细内容放在这里。
EPIC Issue 与 Epic 车道特有的格式（检查点表、Epic 日志、轻量门禁记录、Epic 终止）见 [epic-template.md](epic-template.md)（E 系列）。Epic 内的子 Issue 直接使用本页格式。
...
## WIP 评论（L0-1 — 着手时发到 Issue。4 个字段全部必填）
...
- 无法预测时写 `Files to touch: UNKNOWN` = 仅限串行执行（L0-2）
- 预计沉默时间会超过 72h 的车道，需要在正文中**说明理由**明确写出更长的 staleness 窗口（L0-3 的例外声明。事后延长可以用新评论使其可见）
...
紧接着发布一条新的 WIP 评论（格式如上）。

## RELEASE / HANDED-OFF 评论（L0-3 — 锁的生命周期评论，不是车道状态）

只有**锁的所有者（WIP 中的 agent）**才能发布。其他人解锁的途径只有 stale + TAKEOVER 这一条。
...
## 重新开始检查清单 评论（L0-9 — 恢复／交接／TAKEOVER 的首次写入之前）
...
如需更正，需重新发布一份差替记录并重新打开评审（L1-8）。不允许悄悄修改。
