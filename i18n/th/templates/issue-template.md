...
ต้นฉบับของ field schema สำหรับแต่ละสัญญา (rule ID) ฝั่งเนื้อหาหลัก (docs/01〜03・05〜08) มีเฉพาะข้อความ MUST ขั้นต่ำ ส่วนรายละเอียดจะอยู่ที่นี่
รูปแบบเฉพาะของ EPIC Issue และเลน Epic (ตารางเช็คพอยต์・บันทึก Epic・บันทึก light gate・การสิ้นสุด Epic) อยู่ที่ [epic-template.md](epic-template.md) (ชุด E) ส่วน Issue ลูกภายใน Epic ให้ใช้รูปแบบของหน้านี้ตามเดิม
...
## คอมเมนต์ WIP (L0-1 — โพสต์ลง Issue ตอนเริ่มงาน จำเป็นต้องมีครบทั้ง 4 field)
...
- WIP ที่ขาด 4 field (agent / date / Files to touch / Branch) ถือว่า lock ใช้ไม่ได้ (L0-1)
- กรณีที่คาดการณ์ไม่ได้ ให้ใช้ `Files to touch: UNKNOWN` = เฉพาะแบบ serial เท่านั้น (L0-2)
- เลนที่คาดว่าจะเงียบนานเกิน 72 ชม. ให้เขียนหน้าต่าง staleness ที่ยาวขึ้นไว้ในเนื้อหาอย่างชัดเจน **พร้อมเหตุผล** (ข้อยกเว้นของ L0-3 การขยายเวลาในภายหลังทำได้ด้วยคอมเมนต์ใหม่เพื่อให้มองเห็นได้)

## คอมเมนต์ HOLD (L1-5 — จำเป็นต้องมีครบทั้ง 5 field)
...
หลังจากนั้นให้โพสต์คอมเมนต์ WIP ใหม่ทันที (ตามรูปแบบข้างต้น)

## คอมเมนต์ RELEASE / HANDED-OFF (L0-3 — คอมเมนต์เกี่ยวกับ lifecycle ของ lock ไม่ใช่สถานะเลน)

ผู้ที่สามารถโพสต์ได้มีเพียง **เจ้าของ lock (agent ที่ทำ WIP) เท่านั้น** เส้นทางเดียวที่คนอื่นจะปลด lock ได้คือผ่าน stale + TAKEOVER
...
## คอมเมนต์เช็คลิสต์การกลับมาทำงานต่อ (L0-9 — ก่อนการเขียนครั้งแรกของการกลับมาทำต่อ・การส่งต่องาน・TAKEOVER)
...
กำหนดตายตัวที่ 4 หัวข้อ ห้ามเพิ่ม (L0-9)

## บันทึกความเสร็จสมบูรณ์ (L1-7 — แนบใน PR body เป็น merge gate)
...
การแก้ไขให้เผยแพร่บันทึกฉบับแทนที่ใหม่แล้วเปิด review อีกครั้ง (L1-8) ห้ามแก้แบบเงียบๆ
