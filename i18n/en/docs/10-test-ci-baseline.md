...
- **Tag message format — line 1 = title (120 characters or fewer), line 2 = blank, line 3 onward = body.** Line 1 is a one-line subject naming the release; the release-sync carrier ([templates/ci/](../../../templates/ci/README.md)) uses it **as the Release title verbatim, after trimming surrounding whitespace**. Past 120 characters the Release title falls back to the tag name and cannot be restored afterwards (rewriting an annotated tag rewrites shared history, so it is never done — four releases were measured in [Issue #118](https://github.com/caty-ai/family-dev-handbook/issues/118)). 120 is roughly twice git's subject convention (about 50–72 characters) as an upper bound, and it is **the same value as the implementation in `reusable-release-sync.yml`** — never move one side alone; when the number changes, the implementation and this clause move in the same PR. The body still carries the evidence pointer of the next item, as before
