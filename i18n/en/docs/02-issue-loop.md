...
- **Ratchet ban for follow-up rounds** — from round 2 onward, counted lane-wide, new blocking findings are limited to demonstrated defects (with reproduction or execution-log evidence) or unmet gate criteria. Do not turn new preferences, generalities, or style wishes into late blocking findings (raising them as non-blocking remains fine). A late new blocking finding must satisfy [R-4](09-rejection-rubric.md)'s citation requirement **and additionally** this demonstration — two layered constraints, not the same bar
